...
def convert_to_wav(input_filename):
    converted_filename = os.path.splitext(input_filename)[0] + ".wav"
    subprocess.run(["ffmpeg", "-i", input_filename, converted_filename])
    return converted_filename

def check_audio_for_words(audio_url, words_to_check):
    ...
    temp_filename = f"{unique_filename}.audio"

    # Save audio bytes to the unique temporary file
    with open(temp_filename, "wb") as temp_file:
        temp_file.write(audio_bytes.getvalue())

    # Get the file extension of the downloaded audio
    _, file_extension = os.path.splitext(temp_filename)

    # Convert audio to WAV if needed
    if file_extension.lower() != ".wav":
        converted_temp_filename = convert_to_wav(temp_filename)
        converted_audio_filename = converted_temp_filename
    else:
        converted_audio_filename = temp_filename

    # Initialize the speech recognition recognizer
    recognizer = sr.Recognizer()

    # Recognize speech from the converted audio file
    with sr.AudioFile(converted_audio_filename) as source:
        ...
        try:
            ...
            # Check if the recognized text contains any words from the list
            ...
            if converted_audio_filename != temp_filename:
                os.remove(converted_audio_filename)

def predict_audio(url):
    ...
    result = check_audio_for_words(url, words_to_check)
    return result
